...
>
> 本次审查代码：`/Users/zongcheng/code/3dgs/gsplat`，分支 `gtlr`，提交 `d67a9225ccd8b039fcfd088474156def8f575f86`。原实验副本：服务器 `/media/zc/SSD/GS/lutu/GTLR/gsplat-gtlr/`。
...
> **2026-09-22 审查结论：当前是已有三模块的实验性实现，尚不能认定为完成、可信的论文复现。** 已发现初始化坐标系、子采样近邻索引、深度梯度等确定问题。完整审查见第六至第十节；前五节的服务器运行结果是原有历史记录，本次未重新验证。以下命令保留用于追溯，修复第六节的 P1 问题前不宜直接据此开展效果对比。
...
6. **无偏深度的稳定性仍待验证**：原实验记录早期深度爆炸，采用 step 3000 后启用深度损失（`--depth-start-iter`）及门控、截断缓解。warmup 是本实现的工程选择；当前源码还有坐标、尺度初始化错误，不能将异常全部归因于随机法线，更不能把 loss 变小视为几何已经修复。详见审查 R1–R3、R5。
...
| 无偏深度早期爆炸 | ⚠️ 原记录采用 warmup + min_ray_dot=0.1 + 逐像素 clamp=2.0 缓解；不能认定已修复，见 R3、R5 |
...
审查日期：2026-09-22。范围：上述提交中的 `examples/gtlr/`、`gsplat/strategy/gtlr.py`、相关 Parser、densification、CUDA 像素坐标约定以及 `tests/test_gtlr.py`。本次仅更新文档，没有修改训练实现或启动服务器训练。
...
1. **先保证输入和初始化正确：** 修复 R1、R2、R6、R8、R13；保存统一数据 manifest，检查投影、尺度分位数与初始化空间覆盖。重生成受错误采样影响的工件。
